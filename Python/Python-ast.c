...
ast2obj_stmt(astmodulestate *state, void* _o)
{
    ...
    case Until_kind:
        ...
        if (PyObject_SetAttr(result, state->body, value) == -1)
            goto failed;
        Py_DECREF(value);
        value = ast2obj_list(state, o->v.Until.orelse, ast2obj_stmt);
        if (!value) goto failed;
        if (PyObject_SetAttr(result, state->orelse, value) == -1)
            goto failed;
        Py_DECREF(value);
...
obj2ast_stmt(astmodulestate *state, PyObject* obj, stmt_ty* out, PyArena* arena)
{
    ...
    if (isinstance) {
        ...
        if (tmp == NULL) {
            ...
            int res;
            if (Py_EnterRecursiveCall(" while traversing 'Until' node")) {
                goto failed;
            }
            res = obj2ast_expr(state, tmp, &test, arena);
            Py_LeaveRecursiveCall();
        ...
        *out = Until(test, body, orelse, lineno, col_offset, end_lineno,
                     end_col_offset, arena);
        if (*out == NULL) goto failed;
        return 0;
    }
